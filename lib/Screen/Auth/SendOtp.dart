...
class _SendOtpState extends State<SendOtp> with TickerProviderStateMixin {
  ...
  /*setCodeWithMono() {
    final numericCountryCode =
        context.read<AppSettingsCubit>().getUserCountryCode() ?? '91';

    // Convert numeric country code to ISO 2-letter code
    final isoCountryCode =
        CountryPickerUtils.getCountryByPhoneCode(numericCountryCode).isoCode;

    print("Converted ISO Code: $isoCountryCode");
    return Padding(
        padding: const EdgeInsets.only(top: 45),
        child: Theme(
          data: Theme.of(context).copyWith(
          ...
          child: IntlPhoneField(
            disableLengthCheck:
                true, // don't remove this line this is solution for ignore length check for different country
            style: Theme.of(context).textTheme.titleSmall!.copyWith(
                color: Theme.of(context).colorScheme.fontColor,
                fontWeight: FontWeight.normal),
            controller: mobileController,
            ...
            pickerDialogStyle: PickerDialogStyle(
                ...
                padding: const EdgeInsets.only(left: 10, right: 10),
                searchFieldInputDecoration: InputDecoration(
                  hintText: 'Search country', // Hint text
                  hintStyle: TextStyle(
