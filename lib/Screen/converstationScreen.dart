                            ...
                            ...List.generate(
                              ...
                              UserProvider userProvider =
                                  Provider.of<UserProvider>(context,
                                      listen: false);
                              return userProvider.userId == message.fromId
                                  ? Row(
...
Widget messageWidget(
    BuildContext context, Message message, DateTime messageDate) {
  void openAttachment(
  ...
  return Container(
    ...
    child: Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: userProvider.userId == message.fromId
          ? CrossAxisAlignment.end
          : CrossAxisAlignment.start,
      children: [
        //hide message which is with a file
        (message.message ?? '').toString().isEmpty ||
                (message.mediaFiles?.isNotEmpty ?? false)
            ? const SizedBox()
            : Padding(
                    ...
                    .map((mediaFile) => Container(
                          ...
                          child: ListTile(
                            onTap: () async {
                              openAttachment(
                                  mediaFile: mediaFile,
                                  downloadedInExternalStorage: false);
                            },
                            dense: true,
                            leading: const Icon(Icons.file_copy),
                            title: Text(
                            ...
                            trailing: IconButton(
                                onPressed: () {
                                  openAttachment(
                                      mediaFile: mediaFile,
                                      downloadedInExternalStorage: true);
                                },
        ...
        Padding(
          ...
          child: Text(
            TimeOfDay(hour: messageDate.hour, minute: messageDate.minute)
                .format(context),
            style:
                const TextStyle(fontSize: 9.0, fontWeight: FontWeight.normal),
          ),
        )
      ],
    ),
