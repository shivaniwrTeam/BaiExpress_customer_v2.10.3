...
//  bool isAvailable = true;
  ...
  callApi() async {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (context.read<UserProvider>().userId != '') {
        ...
        print("productIds------${context.read<CartProvider>().productIds}");
        _getOffCart();
        context.read<CartProvider>().productVariantIds =
            (await db.getSaveForLater())!;
        print(
            "productVariantIds------${context.read<CartProvider>().productVariantIds}");
  ...
  razorpayPayment(
    ...
    /*} else {
      if (email == '') {
        setSnackbar(getTranslated(context, 'EMAIL_REQUIRED')!, context);
      } else if (contact == '') {
...
/*    String orderId = DateTime
  ...
  Future<void> _getOffSaveLater() async {
    if (context.read<CartProvider>().productVariantIds.isNotEmpty) {
      isNetworkAvail = await isNetworkAvailable();
      print(
          "productVariantIds2------${context.read<CartProvider>().productVariantIds}");
      if (isNetworkAvail) {
        var parameter = {
          'product_variant_ids':
            ...
            (
              ...
              if (!error) {
                ...
                context
                    .read<CartProvider>()
                    .saveLaterList
                    .clear(); //solve issue of dublicate item in save later list
                if (mounted) context.read<CartProvider>().saveLaterList.clear();
                List<Product> cartList = (data as List)
                    .map((data) => Product.fromJson(data))
                    .toList();
