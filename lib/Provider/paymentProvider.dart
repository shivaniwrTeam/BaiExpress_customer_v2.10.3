...
class PaymentProvider extends ChangeNotifier {
    ...
    if (isNetworkAvail) {
      ...
      try {
        ...
        if (!error) {
          ...
          if (cod) {
            minCodAmount = payment['min_cod_amount'] != null
                ? double.tryParse(payment['min_cod_amount'].toString())
                : null;
            maxCodAmount = payment['max_cod_amount'] != null
                ? double.tryParse(payment['max_cod_amount'].toString())
                : null;
          }
  ...
  Future<String> getPaypalGatewayLink({
    ...
    try {
      var parameter = {AMOUNT: paymentAmount, ORDER_ID: orderID};
